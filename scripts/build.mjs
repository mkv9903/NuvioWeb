...
function displayContentsFallbackPlugin() {
  return {
    postcssPlugin: "display-contents-fallback",
    Declaration(decl) {
      if (decl.prop === "display" && decl.value === "contents") {
        const rule = decl.parent;
        if (!rule || rule.type !== "rule") return;
        const scopedSelectors = rule.selectors.map((s) => `html.no-css-grid ${s}`);
        const fallbackRule = postcss.rule({ selectors: scopedSelectors });
        fallbackRule.append({ prop: "position", value: "absolute" });
        fallbackRule.append({ prop: "top", value: "0" });
        fallbackRule.append({ prop: "left", value: "0" });
        fallbackRule.append({ prop: "width", value: "0" });
        fallbackRule.append({ prop: "height", value: "100%" });
        fallbackRule.append({ prop: "overflow", value: "visible" });
        rule.after(fallbackRule);
      }
    }
  };
}
displayContentsFallbackPlugin.postcss = true;
